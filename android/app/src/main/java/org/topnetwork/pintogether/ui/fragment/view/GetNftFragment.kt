...
import org.topnetwork.pintogether.databinding.FragmentGetNftBinding
import org.topnetwork.pintogether.model.NearBy
import org.topnetwork.pintogether.startNftDetails
import org.topnetwork.pintogether.ui.fragment.vm.GetNftFragmentVm
import org.topnetwork.pintogether.utils.UIUtils
import org.topnetwork.pintogether.widgets.recycler.SpaceItemDecoration
class GetNftFragment: BaseLazyLoadFragment<FragmentGetNftBinding, GetNftFragmentVm>() {

    var mAdapter: GetNftAdapter?= null
    var page = 1
    override fun getLayoutResId(): Int = R.layout.fragment_get_nft

    override fun createViewModel(): GetNftFragmentVm = ViewModelProvider(this).get(
        GetNftFragmentVm::class.java
    )
    ...
    override fun afterViewCreated() {
        ...
        cvb?.swip?.setOnRefreshListener {
            viewModel?.getNftData(AppData.address,1)
    ...
    override fun onFragmentFirst() {
        ...
        viewModel?.getNftData(AppData.address,page)
    }
    ...
    private fun initRv(){
        ...
        mAdapter?.run {
            setOnItemClickListener { adapter, _, position ->
                ...
                startNftDetails((item as NearBy).giftId,2,item)
            }
        }
        cvb?.rv?.run {
            adapter = mAdapter
